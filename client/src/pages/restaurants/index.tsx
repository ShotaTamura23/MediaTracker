...
import { ExternalLink, MapPin, Phone, ChevronDown } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
export default function RestaurantsPage() {
  ...
  const restaurantsByArea = restaurants?.reduce((acc, restaurant) => {
    const area = restaurant.area || "その他"; // Use "その他" (Other) if area is not specified
    if (!acc[area]) {
      acc[area] = [];
    }
    acc[area].push(restaurant);
    return acc;
  }, {} as Record<string, SelectRestaurant[]>) || {};

  // Sort areas alphabetically
  const sortedAreas = Object.keys(restaurantsByArea).sort();
                  ...
                  <AccordionTrigger className="text-xl font-semibold hover:no-underline">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="secondary">
                        {restaurantsByArea[area].length}
                      </Badge>
                    </div>
                  </AccordionTrigger>
                  <AccordionContent>
                    <div className="space-y-4 pt-4">
                      {restaurantsByArea[area].map((restaurant, index) => (
                        <Card
                          key={restaurant.id}
